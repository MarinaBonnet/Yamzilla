...
export const joueurs = {
  1: { nom: "Marina", avatar: "./assets/img/image_manga.jpg" },
  2: { nom: "BotZilla", avatar: "./assets/img/logo.png" },
};
// ─── Avatar sauvegardé (si le joueur est connecté) ─────────────────────────
const savedAvatar = localStorage.getItem("avatar");
if (savedAvatar) {
  joueurs[1].avatar = savedAvatar;
}
...
export function updatePlayerUI(currentPlayer, maxRolls) {
  ...
  if (!joueur) {
    console.warn(
      `⚠️ Joueur ${currentPlayer} introuvable dans la map 'joueurs'`,
    );
    return;
  }
